...
def main():
    ...
    # == build optional model ==
    if cfg.get("ip_clip", None) is not None:
        model_clip_ip = (
            build_module(cfg.ip_clip, MODELS, device_map=device, torch_dtype=dtype).eval().requires_grad_(False)
        )
        log_cuda_memory("ip_clip")
        log_model_params(model_clip_ip)
    ...
    def prepare_inputs(batch):
        ...
        y = batch.pop("text")
        bs = x.shape[0]
        camera_control_config = batch.pop("camera_control_config", None)
        ...
        with nsys.range("encode_text"), timers["encode_text"]:
            inp_ = prepare(model_t5, model_clip, x_0, prompt=y, camera_control_config=camera_control_config)
            inp.update(inp_)
            x_0 = pack(x_0)
        ...
        if "image_clip" in batch:
            inp["face_features"] = batch.pop("face_embedding").to(device, dtype)
            inp["face_features"] = dropout_condition(
                cfg.dropout_ratio.get("face", 0.0),
                inp["face_features"],
                torch.zeros_like(inp["face_features"][0]),
            )

            image_clip = batch.pop("image_clip").to(device, dtype)
            inp["clip_features"] = model_clip_ip(image_clip)
            inp["clip_features"] = dropout_condition(
                cfg.dropout_ratio.get("ip_clip", 0.0),
                inp["clip_features"],
                torch.zeros_like(inp["clip_features"][0]),
            )
    ...
    profiler = ProfilerContext(
        save_path="./log3",  # Profiling 数据保存路径
        record_shapes=True,  # 是否记录张量的形状
        with_stack=True,  # 是否记录调用堆栈
        wait=2,  # 等待步骤数
        warmup=2,  # 热身步骤数
        active=1,  # 激活步骤数
        repeat=1,  # 重复次数
        enable=True,  # 是否启用 profiler
    )
